...
@register.tag
def social_username(parser, token):
    """ Returns the username for this user """
    try:
        tag_name, user_id = token.split_contents()
    except ValueError:
        raise TemplateSyntaxError, '%s is used as "{% social_username <user id> %}"' % token.contents.split()[0]
    return GetUsername(user_id)

class GetUsername(template.Node):
    ...
    def render(self, context):
        try:
            ...
            try:
                profile = SocialProfile.objects.get(user=user_id)
            except SocialProfile.DoesNotExist:
                return ''
            else: return mark_safe(profile.username)
